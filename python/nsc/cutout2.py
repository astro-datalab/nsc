...
def meascutout(meas,obj,size=10,outdir='./'):
    ...
    for i in range(nind):
        ...
        # 13, 8
        plt.plot(np.array([0,0]),np.array([-0.066*npix,0.066*npix])*pixscale,c='white',alpha=0.7)
        plt.plot(np.array([-0.066*npix,0.066*npix])*pixscale,np.array([0,0]),c='white',alpha=0.7)

        # Meas X/Y position
        xmeas,ymeas = wref.all_world2pix(meas['ra'][ind2[i]],meas['dec'][ind2[i]],0)
        plt.scatter([(xmeas-hpix)*pixscale],[(ymeas-hpix)*pixscale],c='r',marker='+',s=20)
