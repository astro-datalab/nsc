...
class Exposure:
    ...
    def process(self):
        ...
        #----------------------------
        for i in [int(sys.argv[5])]: #ktedit:createpsf_test,  only analyze 1 chip!
        #for i in range(1,self.nexten):
            ...
            if 1==1:
                ...
                self.logger.info("number of sources to report = "+str(len(self.chip.cat)))
                chiptimes.add_row([(str(self.origfluxfile).strip().split('/')[-1]).split('.')[0],i,int(self.chip.ccdnum),len(self.chip.cat),int(t1_check-t0)])
                chiptimes = Table(np.unique(chiptimes))
                chiptimes.write(basedir+'lists/nsc_dr3_chiptimes.fits',overwrite=True)
                print("chiptimes table updated")
...
class Chip:
    ...
    def cat(self):
        ...
        if 'CCDNUM' in self.meta.keys():
            ...
            return self._ccdnum
        self.logger.warning('No CAT found')
        return None
...
#            subfile = daobase+str(self.sexiter)+"s.fits"
    ...
    def process(self):
        ...
        while sexiter_endflag==0:
            ...
            print("current length of sexcat = ",len(self.sexcat))
        ...
        self.getapcor()
        self.finalcat()
        print("length of sexcat = ",len(self.sexcat))
