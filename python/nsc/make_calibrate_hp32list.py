...
if __name__ == "__main__":
    ...
    print("version = ",version,", nside = ",nside,", redo = ",redo) #,", npartitions = ",npar,", maxjobs = ",maxjobs)

    # Establish necessary directories 
    # -tempest
    basedir = "/home/x25h971/nsc/instcal/v"+version+"/"   # location of operations
    ...
    list1 = Table.read(basedir+'lists/decam_instcal_list.fits.gz')
    list2 = Table.read(basedir+'lists/mosaic3_instcal_list.fits.gz')
    list3 = Table.read(basedir+'lists/bok90prime_instcal_list.fits.gz')
    for lst in [list1,list2,list3]:
        lst['SB_RECNO'] = lst['SB_RECNO'].astype("U")
    str_data = vstack([list1, list2, list3])
    print(np.shape(str_data))
    ...
    # Get good RA/DEC
    bd = [i for i, (ra, dec) in enumerate(zip(str_data['RA'], str_data['DEC'])) if not (np.isfinite(ra) and np.isfinite(dec))]
