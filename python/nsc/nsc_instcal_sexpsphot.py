...
# Run SExtractor on one FULL DECam/Mosaic3/Bok InstCal image
    ...
    rootLogger.info("Running SExtractor+PSPhot on "+base+" on host="+host)
    ...
    for i in xrange(1,nhdu):
        ...
        if os.path.exists("cat.fits"):
            outcatfile = dir+instcode+"/"+night+"/"+base+"/"+base+"_"+str(ccdnum)+".sex.cat.fits"
            outconfigfile = dir+instcode+"/"+night+"/"+base+"/"+base+"_"+str(ccdnum)+".sex.config"
        ...
        if os.path.exists(slogfile):
            outlogfile = dir+instcode+"/"+night+"/"+base+"/"+base+"_"+str(ccdnum)+".sex.log"
        ...
        # Move the file to final location
        if os.path.exists(base+".cmf"):
            outcatfile = dir+instcode+"/"+night+"/"+base+"/"+base+"_"+str(ccdnum)+".psphot.cat.fits"
            outmdlfile = dir+instcode+"/"+night+"/"+base+"/"+base+"_"+str(ccdnum)+".psphot.mdl.fits"
            outpsffile = dir+instcode+"/"+night+"/"+base+"/"+base+"_"+str(ccdnum)+".psphot.psf.fits"
            # Clobber if it already exists
            if os.path.exists(outcatfile):
                os.remove(outcatfile)
                rootLogger.info("  Copying final catalog to "+outcatfile)
