...
if __name__ == "__main__":
    ...
    while jb_flag==0: # jb_flag = 1 when (jb = ntorun)
        ...
        while nji<nexp:
            ...
            # - Else: add next exposure to batch
                ...
                if lj_status!="NONE":
                    ...
                    odir = basedir+"tmp/"+(expstr['fluxfile'][torun[lsub]].strip()).split(".")[0]+"."
                    for itt in [1,2,3]:
        ...
        for jbsb in ljob_inds:
            ...
            jb_files = [expdir+expstr['fluxfile'][torun[jbsb]].strip(),expdir+expstr['wtfile'][torun[jbsb]].strip(),expdir+expstr['maskfile'][torun[jbsb]].strip()]
            jb_bools = [os.path.exists(jb_files[i]) for i in range(len(jb_files))]
            if False not in jb_bools:
                ...
                job_name = 'nsc_meas_'+str(logtime)+'_'+str(jnum)
            ...
            # - if not, ?????????????? just fuckin skip i guess
                ...
                rootLogger.info("Exposure files not all downloaded, skip for now")
                for fl in np.array(jb_files)[jb_bools]: os.remove(fl)
        #----------------------------------------------------------------------------------
