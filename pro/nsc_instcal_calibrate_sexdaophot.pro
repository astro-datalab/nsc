pro nsc_instcal_calibrate_sexdaophot,expdir,inpref,redo=redo,selfcal=selfcal,saveref=saveref,ncpu=ncpu,stp=stp

; Calibrate catalogs for one exposure, using SE+DAOPHOT photometry

NSC_ROOTDIRS,dldir,mssdir,localdir

; Not enough inputs
if n_elements(expdir) eq 0 then begin
  print,'Syntax - nsc_instcal_calibrate_sexdaophot,expdir,inpref,redo=redo,selfcal=selfcal,saveref=saveref,ncpu=ncpu,stp=stp'
  return
endif
...
if stregex(expdir,'/k4m/',/boolean) eq 1 then begin
  head0 = HEADFITS(expdir+'/'+base+'_1.fits',exten=0)
...
;;  need to use IPP FLAGS/2
goodsources = where(cat.magpsf lt 50,ngoodsources)
printlog,logf,'GOOD SRCS = ',strtrim(ngoodsources,2)

; Measure median seeing FWHM
gdcat = where(cat.magpsf lt 50 and cat.errpsf lt 0.05 and abs(cat.sharp) lt 1 and cat.class_star gt 0.8,ngdcat)
...
For i=0,nchips-1 do begin
  ...
  ;; fwhm_arcsec = sqrt(xx)*2.35*platescale
  gdstars = where(cat2.fwhm_world*3600 lt 2*medfwhm and 1.087/cat2.errpsf gt 50,ngdstars)
  if ngdstars lt 20 then gdstars = where(cat2.fwhm_world*3600 lt 2*medfwhm and 1.087/cat2.errpsf gt 30,ngdstars)
...
'c4d-u': begin
  ...
  mag = cat2.magpsf + 2.5*alog10(exptime)  ; correct for the exposure time
  err = sqrt(cat2.errpsf^2 + ref2.e_nuv^2 + gmagerr2^2)
  ;diff = galex2.nuv-mag
...
'c4d-g': begin
  ...
  ; Use 2MASS and APASS to calibrate
    ...
    mag = cat2.magpsf + 2.5*alog10(exptime)  ; correct for the exposure time
    err = sqrt(cat2.errpsf^2 + ref2.e_apass_gmag^2)  ; leave off JK error for now
...
'c4d-r': begin
  ...
  ; Use 2MASS and APASS to calibrate
    ...
    mag = cat2.magpsf + 2.5*alog10(exptime)  ; correct for the exposure time
    err = sqrt(cat2.errpsf^2 + ref2.e_apass_rmag^2)  ; leave off JK error for now
    ; see nsc_color_relations_stripe82_superposition.pro
...
'c4d-i': begin
  ...
  ; Use GAIA and 2MASS to calibrate
    ...
    mag = cat2.magpsf + 2.5*alog10(exptime)  ; correct for the exposure time
    err = sqrt(cat2.errpsf^2 + gmagerr2^2)  ; leave off the JK error for now
    ; see nsc_color_relations_stripe82_superposition.pro
...
'c4d-z': begin
  ...
  ; Use 2MASS to calibrate  
    ...
    mag = cat2.magpsf + 2.5*alog10(exptime)  ; correct for the exposure time
    err = sqrt(cat2.errpsf^2 + ref2.e_jmag^2)
    ; see nsc_color_relations_stripe82_superposition.pro
...
'c4d-VR': begin
  ...
  ; Take a robust mean relative to GAIA GMAG
  mag = cat2.magpsf + 2.5*alog10(exptime)  ; correct for the exposure time
  err = sqrt(cat2.errpsf^2 + gmagerr2^2)
  model_mag = ref2.gmag
  mstr = {col:col2,mag:float(mag),model:float(model_mag),err:float(err),ccdnum:long(cat2.ccdnum)}
end
; ---- Bok+90Prime g-band ----
...
gdcatmag = where(cat.magpsf lt 50,ngd)
cat[gdcatmag].cmag = cat[gdcatmag].magpsf + 2.5*alog10(exptime) + expstr.zpterm
cat[gdcatmag].cerr = sqrt(cat[gdcatmag].errpsf^2 + expstr.zptermerr^2)  ; add calibration error in quadrature
