...
;
; USAGE:
;  IDL>cat = getrefcat_v3(cenra,cendec,radius,refcat,file=file,saveref=saveref)
;
; By D. Nidever  Sep 2017
;-

function getrefcat_v3,cenra,cendec,radius,refcat,count=count,file=file,saveref=saveref,silent=silent,logfile=logfile
...
if n_elements(cenra) eq 0 or n_elements(cendec) eq 0 or n_elements(radius) eq 0 or $
  ...
  print,'Syntax - cat = getrefcat_v3(cenra,cendec,radius,refcat,file=file,saveref=saveref)'
  return,-1
endif
