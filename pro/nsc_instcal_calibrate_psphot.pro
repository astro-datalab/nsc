pro nsc_instcal_calibrate_psphot,expdir,inpref,redo=redo,selfcal=selfcal,saveref=saveref,ncpu=ncpu,stp=stp

; Calibrate catalogs for one exposure, using PSPhot photometry

NSC_ROOTDIRS,dldir,mssdir,localdir

; Not enough inputs
if n_elements(expdir) eq 0 then begin
  print,'Syntax - nsc_instcal_calibrate_psphot,expdir,inpref,redo=redo,selfcal=selfcal,saveref=saveref,ncpu=ncpu,stp=stp'
  return
endif
...
if stregex(expdir,'/k4m/',/boolean) eq 1 then begin
  ;; mld.fits HDU0 has the original header
  head0 = HEADFITS(expdir+'/'+base+'_1.psphot.mdl.fits',exten=0)
...
goodsources = where(finite(cat.psf_inst_mag_sig) eq 1,ngoodsources)
...
gdcat = where(finite(cat.psf_inst_mag_sig) eq 1 and cat.psf_inst_mag_sig lt 0.05 and cat.psf_qf gt 0.9,ngdcat)
medfwhm = median( (cat[gdcat].psf_fwhm_maj+cat[gdcat].psf_fwhm_min)*0.5 * abs(cat[gdcat].pltscale) )
...
For i=0,nchips-1 do begin
  ...
  qcuts1 = where(finite(cat1b.psf_inst_mag_sig) eq 1,nqcuts1)
  ...
  gdstars = where(cat2.psf_fwhm_maj*abs(cat2.pltscale) lt 2*medfwhm and 1.087/cat2.psf_inst_mag_sig gt 50,ngdstars)
  if ngdstars lt 20 then gdstars = where(cat2.psf_fwhm_maj*abs(cat2.pltscale) lt 2*medfwhm and 1.087/cat2.psf_inst_mag_sig gt 30,ngdstars)
